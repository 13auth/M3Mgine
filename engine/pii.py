...
# Çıktıda AÇIKÇA ifşa edilen PII = ihlal. enforce bunları HARD forbid-pattern olarak kullanır:
# scrub'dan ÖNCE çalışır (soft judge'a giderken PII redakte olur, yakalanmaz) ve tek literal
# örnek yerine KATEGORİ deseni olduğu için GÖRÜLMEYEN her instance'a (başka kart/TCKN/IBAN)
# genelleşir. compiler, PII düzeltmelerinde forbid_patterns'a bunları enjekte eder.
_CARD = re.compile(r"(?<!\d)(?:\d[ -]?){12,18}\d(?!\d)")  # 14-19 haneli kart (gruplu/bitişik)


def forbid_patterns() -> list[str]:
    """PII kategorisi için kanonik forbid regex'leri (kart / TCKN / IBAN / TR telefon)."""
    return [_CARD.pattern, _TCKN.pattern, _IBAN_TR.pattern, _PHONE_TR.pattern]
