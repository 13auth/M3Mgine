...
def _compliance(a):
    rows = store.compliance_by_rule(a.tenant)
    if not rows:
        print("(henüz ölçüm yok — check/eval çalıştır)")
        return 0
    print(f"== per-rule uyum (tenant={a.tenant}) ==")
    for r in sorted(rows, key=lambda x: (x["compliance"] if x["compliance"] is not None else 1)):
        comp = f"{r['compliance']:.0%}" if r["compliance"] is not None else "-"
        print(f"  {comp:>5}  {r['rule_id']}  ({r['passed']}/{r['checks']} geçti, {r['violations']} ihlal)")
    return 0
...
def main():
    ...
    p = sub.add_parser("compliance", help="per-rule canlı uyum oranları")
    p.add_argument("--tenant", required=True); p.set_defaults(func=_compliance)
