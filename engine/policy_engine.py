...
def enforce(output: str, rules: list[dict] | None = None, project: str | None = None,
    ...
    # SOFT kuralların deterministik ACCELERATOR'ı: forbid/require pattern'i varsa zero-LLM
    # önce dene (run_soft'tan BAĞIMSIZ). Yakalarsa judge'a gerek yok (maliyet + kesinlik).
    # Böylece bir kural soft kalsa bile BİLİNEN literal ihlaller anında, GÖRÜLMEYENLER judge ile
    # yakalanır -> genelleme artık modelin hard/soft seçimine bağlı değil (hibrit kural).
    soft_pattern_caught: set[str] = set()
    for rule in soft:
        uf = [p for p in rule.get("forbid_patterns", []) if _pattern_ok(p)]
        ur = [p for p in rule.get("require_patterns", []) if _pattern_ok(p) and not _always_satisfied(p)]
        if not (uf or ur):
            continue
        vs = _check_hard(output, rule)
        if vs:
            res.checked += 1
            res.violations.extend(vs)
            res.evaluated.append({"rule_id": rule["id"], "passed": False})
            soft_pattern_caught.add(rule["id"])

    if not run_soft:
        # soft judge kapalı: pattern'le yakalanMAYANLAR DEĞERLENDİRİLEMEDİ -> unevaluated (fail-closed)
        for r in soft:
            if r["id"] in soft_pattern_caught:
                continue
            res.unevaluated.append({"rule_id": r["id"], "severity": _sev(r),
                                    "reason": "soft_disabled"})
        return res

    # judge'a yalnızca accelerator'ın yakalaMADIĞI soft kurallar girer.
    # soft pre-filter: YÜKSEK/KRİTİK soft kurallar ASLA sessizce atlanmaz.
    judge_soft = [r for r in soft if r["id"] not in soft_pattern_caught]
    BLOCKING = ("high", "critical")
    must_run_all = [r for r in judge_soft if _sev(r) in BLOCKING]
    budgetable = [r for r in judge_soft if _sev(r) not in BLOCKING]
    selected = []
