...
Reproducible, held-out quality measurements. The first three target the **enforcement loop**
— what most memory tools don't have, and therefore don't measure. Full report:
[`benchmarks/REPORT.md`](engine/benchmarks/REPORT.md) · raw numbers:
[`results.json`](engine/benchmarks/results.json).
...
<sub>`google/gemini-2.5-flash-lite`, 3-run average. Datasets are small and deliberately
adversarial (early-stage); numbers move run-to-run on a judge model — treat as directional.
Reproduce: `cd engine && python bench.py`.</sub>
